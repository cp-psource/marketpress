...
Plugin URI: https://cp-psource.github.io/marketpress/psource.html
Description: Zeigt MarketPress-Statistiken an
Version: 0.4.2
Author: PSOURCE
*/
...
function mp_st_admin_menu() {
  add_dashboard_page(
    __('Verkaufsstatistik', 'mp_st'),   // Seitentitel
    __('Shopstatistik', 'mp_st'),       // Menütitel
    'administrator',                    // Benutzerrolle, die Zugriff hat
    'mp_st',                            // Seitenslug
    'mp_st_page',             // Callback-Funktion für die Seitenausgabe
    30                                  // Position im Menü
  );
}
...
function mp_st_page() {
  ...
  function mp_st_sales_by_price( $echo = true, $post_id = NULL, $label = true ) {
    ...
    if ((is_array($meta["mp_price"]) && count($meta["mp_price"]) >= 1) || !empty($meta["mp_file"])) {
        if ($meta["mp_is_sale"]) {
            $price = (float) $meta["mp_sale_price"][0]; // Preis als float konvertieren
        } else {
            $price = (float) $meta["mp_price"][0]; // Preis als float konvertieren
        }
    } else {
        $price = 0; // Wenn keine Preise gefunden werden, 0 zurückgeben
    }

    $sales = (int) $meta["mp_sales_count"][0]; // Verkaufszahlen als integer konvertieren
    ...
    if ($echo) {
        echo json_encode($stats); // Statistiken als JSON ausgeben, wenn $echo true ist
    } else {
        return $stats; // Statistiken zurückgeben, wenn $echo false ist
    }
  } ?>
                ...
                <?php
                    ...
                    if ($custom_query->have_posts()) {
                        while ($custom_query->have_posts()) {
                            ...
                            $post_id = get_the_ID();
                            $stats = mp_st_sales_by_price(false, $post_id, false); // Statistiken für jedes Produkt abrufen
                            if ($stats !== false) {
                                echo ", [" . $stats['price'] . ", " . $stats['sales'] . "]"; // Numerische Werte für Preis und Umsatz übergeben
                            }
                        }
                        wp_reset_postdata();
                    }
